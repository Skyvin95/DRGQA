...
class CNN_block(nn.Module):
    ...
    def __init__(self, in_dim, out_dim, stride=1): 
        ...
        self.depth_conv1 = Depthwise_conv(in_dim, out_dim)
        self.depth_conv2 = Depthwise_conv(out_dim, out_dim, stride=stride)
    ...
    def forward(self, x, x_t=None):
        ...
        x = self.bn1(x)
        x = self.relu1(x)
        x = self.depth_conv2(x) if x_t is None else self.depth_conv2(x + x_t)
        x = self.bn2(x)
...
class ConvTransBlock(nn.Module):
    ...
    def forward(self, x, x_t1, x_t2, tex, color, com):
        ...
        x_st1 = self.squeeze_block(x, x_t1)
        x_t1 = self.trans_block_mix(x_st1 + x_t1, tex)
        x_t_r1 = self.expand_block(x_t1, H // self.dw_stride, W // self.dw_stride)

        x_st2 = self.squeeze_block(x, x_t2)
        x_t2 = self.trans_block_mix(x_st2 + x_t2, color)
        x_t_r2 = self.expand_block(x_t2, H // self.dw_stride, W // self.dw_stride)
...
class DRGQA(nn.Module):
    ...
    def __init__(self, patch_size=32, in_chans=3, num_classes=1, base_channel=64,
        ...
        self.convt = conv_block(in_dim=1, out_dim=64)
        self.convc = conv_block(in_dim=1, out_dim=64)
        self.down_block_tex = Downsample(in_dim=64, out_dim=embed_dim, dw_stride=8)
    ...
    def forward(self, x, tex, color):
        B = x.shape[0]
        cls_tokens = self.cls_token.expand(B, -1, -1)

        # stem stage
        x_base = self.convx(x) 
        tex_base = self.convt(tex)
        color_base = self.convc(color)
        tex_q = self.down_block_tex(tex_base)
        color_q = self.down_block_color(color_base)

        # res * enhanced
        com = tex_base * x_base + color_base * x_base
        ...
        for i in range(1, self.fin_stage):
            x, x_t1, x_t2 = eval('self.conv_trans_' + str(i))(x, x_t1, x_t2, tex_q, color_q, com)
